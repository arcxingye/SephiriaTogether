...
namespace SephiriaTogether
{
    internal static class MenuText
    {
        private static readonly Dictionary<string, string[]> Text = new Dictionary<string, string[]>
        {
            ...
            ["DirectMode"] = new[] { "使用 IP 传输", "使用 IP 傳輸", "Use IP transport", "IP 전송 사용", "IP通信を使用" },
            ["DirectConnectHelp"] = new[] { "IP 传输会在首次网络会话前安装。Steam 在线版和离线版都可在未建房时切换；进入房间或游戏后不会热切换。选存档进入大厅后，创建房间会显示监听端口，加入会弹出 IP 输入框，刷新会查询局域网与同网段主机。", "IP 傳輸會在首次網路工作階段前安裝。Steam 線上版與離線版都可在尚未建立房間時切換；進入房間或遊戲後不會熱切換。選擇存檔進入大廳後，建立房間會顯示監聽連接埠，加入會彈出 IP 輸入框，重新整理會查詢區域網路與同網段主機。", "The IP transport is installed before the first network session. It can be toggled on Steam or offline before a room is created; an active room or game never hot-switches transport. Create Room shows the listening port, Join opens an IP prompt, and Refresh queries LAN and same-subnet hosts.", "IP 전송은 첫 네트워크 세션 전에 설치됩니다. Steam과 오프라인 모두 방을 만들기 전에 전송을 켜거나 끌 수 있으며, 방이나 게임 세션이 시작되면 전송 방식을 바꾸지 않습니다. 방 만들기는 수신 포트를 표시하고 참가 버튼은 IP 입력을 열며 새로고침은 LAN과 같은 서브넷 호스트를 검색합니다.", "IP通信は最初のネットワークセッション前に導入されます。Steamとオフラインのどちらでもルーム作成前に切り替えられ、ルームやゲーム中は通信方式を切り替えません。部屋作成は待受ポートを表示し、参加はIP入力を開き、更新はLANと同一サブネットのホストを検索します。" },
            ["DirectPort"] = new[] { "端口（建房前可修改）", "連接埠（建立房間前可修改）", "Port (before hosting)", "포트 (방 만들기 전 변경 가능)", "ポート（ホスト開始前に変更可能）" },
            ["IpOfflineAutomatic"] = new[] { "检测到 Steam 未登录；离线版默认启用 IP 传输，也可以在此关闭。", "偵測到 Steam 未登入；離線版預設啟用 IP 傳輸，也可以在此關閉。", "Steam is offline; IP transport is enabled by default and can be turned off here.", "Steam 오프라인 상태입니다. IP 전송은 기본적으로 켜져 있으며 여기서 끌 수 있습니다.", "Steam未ログインです。IP通信はデフォルトで有効ですが、ここで無効にできます。" },
            ...
            ["GameVersionMismatchWarning"] = new[] { "游戏版本不一致：本机 {0} / 对方 {1}。游戏更新后网络协议不同，双方需使用相同游戏版本才能联机。", "遊戲版本不一致：本機 {0} / 對方 {1}。遊戲更新後網路協定不同，雙方需使用相同遊戲版本才能連線。", "Game version mismatch: local {0} / remote {1}. Updated builds change the network protocol, so both sides must run the same game version to connect.", "게임 버전 불일치: 내 버전 {0} / 상대 {1}. 업데이트된 빌드는 네트워크 프로토콜이 달라 양쪽 게임 버전이 같아야 연결할 수 있습니다.", "ゲームバージョン不一致：自分{0} / 相手{1}。更新後はネットワークプロトコルが異なるため、同じゲームバージョンでないと接続できません。" },
            ["ModVersionMismatchWarning"] = new[] { "Mod 版本不一致：本机 {0} / 对方 {1}。仍可继续联机，部分功能可能不可用。", "Mod 版本不一致：本機 {0} / 對方 {1}。仍可繼續連線，部分功能可能無法使用。", "Mod version mismatch: local {0} / remote {1}. You can continue; some features may be unavailable.", "Mod 버전 불일치: 내 버전 {0} / 상대 {1}. 계속 연결할 수 있지만 일부 기능이 작동하지 않을 수 있습니다.", "Modバージョン不一致：自分{0} / 相手{1}。接続は続行できますが一部機能が使えない場合があります。" },
