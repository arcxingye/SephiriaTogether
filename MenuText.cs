...
namespace SephiriaTogether
{
    internal static class MenuText
    {
        private static readonly Dictionary<string, string[]> Text = new Dictionary<string, string[]>
        {
            ...
            ["CloneBots"] = new[] { "克隆 Bot", "複製 Bot", "Clone Bots", "복제 Bot", "クローン Bot" },
            ["CloneBotsHelp"] = new[] { "创建本局临时分身，复制当前武器、武器形态、有效战斗属性和外观；分身只跟随并协助战斗，不选奖励、不保存。所有真人客户端需安装相同版本。", "建立本局臨時分身，複製目前武器、武器形態、有效戰鬥屬性與外觀；分身只跟隨並協助戰鬥，不選獎勵、不儲存。所有真人用戶端需安裝相同版本。", "Create a temporary clone of the current weapon, weapon form, effective combat stats, and appearance. It only follows and fights, never chooses rewards or saves. Every real client must use the same version.", "현재 무기, 무기 형태, 유효 전투 능력치와 외형을 복제한 임시 Bot을 만듭니다. 따라다니며 전투만 돕고 보상을 선택하거나 저장하지 않습니다. 모든 실제 클라이언트가 같은 버전을 사용해야 합니다.", "現在の武器・武器形態・有効戦闘能力・外見を複製した一時Botを作成します。追従と戦闘支援のみ行い、報酬選択や保存はしません。全実プレイヤーが同じ版を使用する必要があります。" },
            ["CloneSources"] = new[] { "选择克隆来源", "選擇複製來源", "Choose clone source", "복제 대상 선택", "クローン元を選択" },
            ["ActiveCloneBots"] = new[] { "当前分身", "目前分身", "Active clones", "활성 분신", "現在のクローン" },
            ["NoCloneBots"] = new[] { "当前没有分身。", "目前沒有分身。", "No clones are active.", "활성 분신이 없습니다.", "現在クローンはいません。" },
            ["CloneLocalPlayer"] = new[] { "创建分身", "建立分身", "Create clone", "분신 생성", "クローン作成" },
            ["RemoveCloneBot"] = new[] { "移除", "移除", "Remove", "제거", "削除" },
            ["RemoveAllCloneBots"] = new[] { "移除全部分身", "移除全部分身", "Remove all clones", "모든 복제 제거", "全クローンを削除" },
            ...
            ["AutoPilotHelp"] = new[] { "开启后自动移动、战斗、拾取和前进，并按当前武器及其强化调整攻击方式和距离。会触发并清空当前楼层的全部战斗区域与敌对怪物，领取可用奖励后才前往下一关；优先救援可到达的队友。可从菜单或已绑定的快捷键关闭。", "開啟後自動移動、戰鬥、拾取和前進，並依目前武器及其強化調整攻擊方式與距離。會觸發並清空目前樓層的全部戰鬥區域與敵對怪物，領取可用獎勵後才前往下一關；優先救援可到達的隊友。可從選單或已綁定的快速鍵關閉。", "Automatically move, fight, collect, and advance while adapting to the equipped weapon and upgrades. It triggers and clears every battle area and hostile on the current floor, collects available rewards, and only then advances. Reachable teammates are rescued first. Stop it from the menu or a bound shortcut.", "현재 무기와 강화에 맞춰 자동 이동, 전투, 수집과 진행을 수행합니다. 현재 층의 모든 전투 구역과 적을 활성화해 처치하고 가능한 보상을 받은 뒤에만 다음 층으로 이동합니다. 도달 가능한 팀원을 우선 구조하며 메뉴 또는 지정된 단축키로 중지할 수 있습니다.", "現在の武器と強化に応じて移動、戦闘、回収、進行を自動化します。現在フロアの全戦闘区域を起動して敵対ユニットを全滅させ、取得可能な報酬を回収してから次へ進みます。到達可能な仲間を優先して救助し、メニューまたは割り当て済みキーから停止できます。" },
            ...
            ["AutoPilotBannerMenu"] = new[] { "自动游玩中 · 从菜单关闭", "自動遊玩中 · 從選單關閉", "AUTOPLAY · STOP FROM MENU", "자동 플레이 · 메뉴에서 중지", "自動プレイ中 · メニューで停止" },
            ...
            ["ManualArrangeUnavailable"] = new[] { "当前无法整理：请确认玩家与背包已加载、处于非战斗状态且没有正在保存。", "目前無法整理：請確認玩家與背包已載入、處於非戰鬥狀態且沒有正在儲存。", "Inventory arrangement is unavailable while loading, fighting, or saving.", "로딩, 전투 또는 저장 중에는 인벤토리를 정리할 수 없습니다.", "ロード中、戦闘中、保存中はインベントリを整理できません。" },
            ["ManualArrangeRequested"] = new[] { "已请求房主整理背包。", "已請求房主整理背包。", "Inventory arrangement requested from the host.", "호스트에 인벤토리 정리를 요청했습니다.", "ホストにインベントリ整理を要求しました。" },
