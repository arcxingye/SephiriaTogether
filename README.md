...
The menu is organized into Rules, Autopilot, Compensation, Diagnostics, and History tabs. Rules contains host multiplayer, scaling, and player controls; Autopilot contains only local AFK controls and presets and is identical for hosts and clients. The menu shortcut is configurable through `Interface/MenuShortcut`, defaulting to `F8`; modifier keys are supported by the BepInEx `KeyboardShortcut` format.
...
The optional automatic guard and parry assistant is input-only. It supports the defensive sword-and-shield guard and dagger parry, predicts threats from nearby enemy windup/fire phases, hostile active melee collisions, and hostile projectiles moving toward the player, then presses vanilla secondary attack before impact. Sword-and-shield holds guard while the threat remains; dagger issues a timed parry request. Vanilla movement state, MP cost, guard availability, animation events, perfect-guard/parry windows, latency handling, and server authority still decide whether defense succeeds. The mod never starts guard/invulnerability directly and never changes damage in an ApplyDamage hook.
...
Autopilot choice handling is local and configurable in F8: prefer presets, prefer favorites, or always wait. Reward and weapon-upgrade presets have separate vanilla-name dropdowns. The weapon dropdown reads the equipped weapon and shows its complete reachable enhancement tree with upgrade-depth labels. A final upgrade can be selected at the start; at an earlier Anvil, autopilot chooses the immediate candidate whose descendant tree contains that highest-priority target. Reopening F8 refreshes the tree after the weapon changes. Added choices appear as ordered removable tags; their order is the matching priority. If a reward has no preferred match, autopilot randomly chooses among its highest-rarity candidates. Always wait leaves every choice unclaimed. Weapon upgrades have no rarity and remain preset-only, so an unmatched Anvil is left unclaimed rather than choosing an irreversible build path at random.
...
The default next-floor priority is Anvil, Inventory Expansion, the game's localized Charm item type (神器 in Simplified Chinese), then EXP. Existing installations with an empty floor priority receive this default once; later edits or an intentionally empty list are preserved.
...
```powershell
.\scripts\package.ps1 -Version 3.5.0
```
