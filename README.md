...
The menu is organized into Rules, Compensation, Diagnostics, History, Saves, and Transfer tabs. Rules contains host multiplayer, scaling, player controls, and a manual inventory-arrangement action. The arrangement button invokes the game's original best-Charm-level arranger only when clicked, so it may move ordinary items, Charms, and Stone Tablets and rotate eligible Tablets; there is no automatic trigger in Sephiria Together. Transfer lets a modded sender choose any other online player, enter a whole Leaf amount, and confirm it. The host validates the sender identity, target, positive amount, balance, overflow, and request rate before atomically moving the Leaves; recipients do not need the mod. The menu shortcut is configurable through `Interface/MenuShortcut`, defaulting to `F8`; modifier keys are supported by the BepInEx `KeyboardShortcut` format.
